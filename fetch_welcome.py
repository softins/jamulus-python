...
def main():
    ...
    for addr in args.server:
        jc.sendto(addr, "CLM_REQ_WELCOME_MESSAGE")
    ...
    while True:
        ...
        elif key == "CLM_SERVER_FEATURES":
            # server features received
            print("received server features")
            print(format_features(values))
            #break

        elif key == "CLM_WELCOME_MESSAGE":
            # server welcome message received
            print("received welcome message")
            print(format_text(values))
            #break
